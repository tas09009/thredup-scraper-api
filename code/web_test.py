...
url_original = 'https://www.thredup.com/petite?chars_sleeve_length=short%20sleeve&department_tags=petite&search_tags=women-tops%2Cwomen-tops-button-down-shirts%2Cwomen-tops-knit-tops&sizing_id=750%2C755%2C756%2C765&skip_equivalents=true&state=listed&color_names=tan'
# url = url_original[:-1]
...
for page_number in range(1,2): # Everytime range increases, items increase by 50. Ordered by "Newest first"
    ...
    hrefs = []
    product_list = []
    grid_products = main_page_items.findAll('div', {'class': 'u-bg-white u-text-black u-inline-block u-p-1x u-relative u-overflow-hidden _2ApPUPOe-XfQ2bYtL-ISFU _3LiDWtsVjVBIr6EyFft3an grid-item'})
    for i in grid_products:
        product = i.find('a', {'class': 'u-inset-0 u-absolute _3zn_97lKU4aIqPvlkiN4_h'}).get('href')
    ...
    for link in hrefs:
        ...
        image_search = product_page_soupified.find('a', {'class': 'u-relative u-w-full _YFsbsE3S64aEw-MiDk2C'}).get('href')
        ...
        product_materials = []
        materials_search = product_page_soupified.findAll('li', {'class': 'c_28Qskcy2Nmx1ocA4jvV6ct'})
